...
void Glyphs::Init(v8::Handle<v8::Object> target) {
    ...
    NODE_SET_PROTOTYPE_METHOD(constructor, "rects", Rects);
...
NAN_METHOD(Glyphs::Range) {
    ...
    if (args.Length() < 4 || !args[3]->IsBoolean()) {
        return NanThrowTypeError("bitmap must be a boolean");
    }
    ...
    v8::Local<v8::Boolean> bitmap(args[3]->ToBoolean());
    ...
    baton->range = *range;
    baton->chars = chars;
    baton->bitmap = bitmap->BooleanValue();

    uv_work_t *req = new uv_work_t();
    req->data = baton;

    int status = uv_queue_work(uv_default_loop(), req, AsyncRange, (uv_after_work_cb)After);
    assert(status == 0);

    NanReturnUndefined();
}

NAN_METHOD(Glyphs::Rects) {
    NanScope();
    ...
    v8::String::Utf8Value fontstack(args[0]->ToString());
    v8::Local<v8::Array> charsArray = v8::Local<v8::Array>::Cast(args[1]);
    ...
    int status = uv_queue_work(uv_default_loop(), req, AsyncRects, (uv_after_work_cb)After);
    assert(status == 0);
...
void Glyphs::AsyncRange(uv_work_t* req) {
    ...
    try {
        baton->glyphs->glyphs.Range(baton->fontstack, baton->range, baton->chars, baton->bitmap);
...
void Glyphs::AsyncRects(uv_work_t* req) {
    Baton* baton = static_cast<Baton*>(req->data);

    try {
        baton->glyphs->glyphs.Rects(baton->fontstack, baton->chars);
    } catch(const std::runtime_error &e) {
        baton->error = true;
        baton->error_name = e.what();
        return;
...
void Glyphs::After(uv_work_t* req) {
    NanScope();
    Baton* baton = static_cast<Baton*>(req->data);
